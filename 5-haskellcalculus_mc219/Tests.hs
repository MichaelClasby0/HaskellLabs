...
showTests
  = [ (BinApp Add (Val 1.0) (UnApp Log (Id "x"))) ==>
    ...
    , (BinApp Mul (Val 4.0) (BinApp Add (Id "x") (UnApp Neg (Id "y")))) ==>
        "(4.0*(x+-(y)))"
    , (diff (BinApp Add (Val 1.0) (UnApp Log (Id "x"))) "x") ==>
        "(0.0+(1.0/x))"
    , (diff (2 * sin (x + log y)) "x") ==>
        "((2.0*(cos((x+log(y)))*(1.0+(0.0/y))))+(0.0*sin((x+log(y)))))"
    ]
...
allTestCases
  = [ floatTestCase "lookUp"          (uncurry lookUp)     lookUpTests
    , floatTestCase "eval"       (uncurry eval)       evalTests
    , testCase "showExp"         (showExp)            showTests
